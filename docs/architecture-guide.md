...
2026-09-21 補充：下文保留最初架構說明，示意圖數量與載入方式已更新。現為 27 件原作與 2 張展示背板；網站 GLB 只內嵌小預覽，`artwork-textures.ts` 在可操作後逐步載入獨立清晰貼圖。完整資產流程與最新配置請以 [README](../README.md) 及 [效能與空間校正](loading-and-space.md) 為準。
...
| `blender/build_gallery.py` | 建築、無框畫立體網格、識別碼、資產匯出 | 可重跑的資產產生器 |
...
作品使用單一有厚度的 Mesh，包含正面、四個包覆側面與背面，沒有額外邊框。UV 將圖像包到側面，頂點顏色降低側面亮度以呈現折角；前端以啟用 `vertexColors` 的 `MeshBasicMaterial` 保留這些色彩。柱畫 UV 對位至完整現場照片內的畫作四角，詳情則顯示原始照片。
...
初始導覽位置由 `gallery.json` 的 `layout.entrance` 提供，目前是 `[1.45, 1.65, -2.45]`；位置在服務台旁、隔間左側門口內側，`yaw=π/2` 朝柱子與展場觀看，服務台在右側。這不是 CSS 的 left／top，也不是相對目前視窗的像素。
...
顯示幾何與碰撞幾何是不同資料。GLB 有樑、管線、無框畫折角等細節；`gallery.json` 的 colliders 包含入口隔間、開啟的門扇、兩根方柱、服務台、簽到桌及三張椅子；門洞保留可通行空間。外牆由可站立中心的 `bounds` 限制，並非每個可見 Mesh 都會自動阻擋行走。
