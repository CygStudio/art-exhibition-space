...
export interface Layout {
  ...
  unusedRoom: Footprint
  serviceDesk: Footprint
  guestbook: Footprint
  window: Footprint
  columns: Footprint[]
  entrance: { position: [number, number, number]; yaw: number }
}
...
export function drawFloorPlan(group: SVGGElement, layout: Layout) {
  ...
  const label = mapPoint(layout.unusedRoom.x, layout.unusedRoom.z, layout)
  ...
  group.innerHTML = rect(room, '#e5e0d6', '#9c978c')
    + rect(layout.unusedRoom, '#c9c6c1', '#817e78')
    ...
    + `<text x="${label.x}" y="${label.y}" text-anchor="middle" font-size="5.8" fill="#57534d">未使用</text>`
}
