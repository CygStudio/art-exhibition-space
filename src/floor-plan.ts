...
export interface Layout {
  ...
  window: Footprint
  columns: Footprint[]
  entrance: { position: [number, number, number]; yaw: number; door: { x: number; z: number; width: number; height: number } }
}
...
export function drawFloorPlan(group: SVGGElement, layout: Layout) {
  ...
  const hinge = mapPoint(door.x - door.width / 2, door.z, layout)
  ...
  group.innerHTML = rect(room, '#e5e0d6', '#9c978c')
    ...
    + `<path d="M${hinge.x} ${hinge.y}h${radius}" stroke="#eeeae2" stroke-width="3"/>`
    + `<path d="M${hinge.x} ${hinge.y}v${-radius} A${radius} ${radius} 0 0 1 ${hinge.x+radius} ${hinge.y}" fill="none" stroke="#68645f" stroke-width="1.3"/>`
    + `<text x="${hinge.x+radius/2}" y="${hinge.y+7}" text-anchor="middle" font-size="5.8" fill="#57534d">入口</text>`
}
